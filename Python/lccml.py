...
class LCCML:
    ...
    #                                                 #
    # ----------------------------------------------- #
    def __predictChain2(self, x, chainIndex):
        """
        Predict using a single label cluster chain of the ensemble.
        """
        chain_X = x.copy()
        pred_dfs = []  # acumular previsões aqui
        ...
        for model in self.chains[chainIndex]:
            ...
            pred_df = pd.DataFrame(pred_proba, columns=model.labelName_)
            pred_dfs.append(pred_df)  # salva o resultado do cluster atual

            # Atualiza as features com as predições do cluster atual
            chain_X = pd.concat([chain_X, pred_df], axis=1)

        # Junta todas as previsões de uma vez — muito mais rápido e sem fragmentação
        predictions = pd.concat(pred_dfs, axis=1)
